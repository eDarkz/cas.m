...
function proportionalForPeriod(hireDate: string, from: string, to: string) {
  ...
  for (let serviceYear = Math.max(1, fromYears + 1); serviceYear <= toYears + 1; serviceYear += 1) {
    ...
    segments.push({
      ...
      proportional_days: roundVacationDays(proportional)
    });
  }
  ...
  return {
    from,
    to,
    proportional_days: roundVacationDays(total),
    segments
  };
}
...
async function countVacationDays(
  ...
  const workDays = weekdayWorkMap(employee);
  ...
  while (current <= end) {
    ...
    const isWorkday = includeWeekends ? true : workDays[current.getUTCDay()];
    const isHoliday = holidays.has(date);
    if (isWorkday && !isHoliday) count += 1;
...
router.get('/employees', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const employees = await withConn(async (conn) => {
      ...
      if (q.q) {
        where.push('(full_name LIKE ? OR employee_number LIKE ? OR email LIKE ? OR position LIKE ?)');
        args.push(`%${q.q}%`, `%${q.q}%`, `%${q.q}%`, `%${q.q}%`);
      }
      const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';
      const [rows]: any = await conn.query(
        `SELECT * FROM vac_employees ${whereSql} ORDER BY active DESC, full_name ASC LIMIT ?`,
        [...args, q.limit]
      );
...
router.post('/employees', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const employee = await withConn(async (conn) => {
      ...
      const [[created]]: any = await conn.query('SELECT * FROM vac_employees WHERE id=?', [id]);
    ...
    const accrual = await withConn(async (conn) => {
      ...
      return {
        ...
        current_service_period: {
          ...
          proportional_days_generated: roundVacationDays(currentProportional),
          remaining_proportional_days: roundVacationDays(currentLegalDays - currentProportional)
        },
        calendar_year: {
          year: calendarYear,
  ...
  try {
    ...
    const request = await withConn(async (conn) => {
      ...
      const requestedDays = body.requested_days == null
        ? await countVacationDays(conn, employee, body.start_date, body.end_date, body.include_weekends, body.include_holidays)
        : body.requested_days;
...
router.patch('/requests/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const request = await withConn(async (conn) => {
      ...
      const includeWeekends = body.include_weekends ?? toBool(current.include_weekends);
      ...
      const requestedDays = body.requested_days === undefined
        ? await countVacationDays(conn, current, startDate, endDate, includeWeekends, includeHolidays)
        : body.requested_days;
      ...
      if (body.include_weekends !== undefined) add('include_weekends=?', body.include_weekends ? 1 : 0);
